...
//  NAVBAR
  document.addEventListener("DOMContentLoaded", function() {
    ...
    .then(data => {
      ...
      document.getElementById('navbar_main').style.backgroundColor = bodyBackgroundColor;
    });
  });
...
const schoolNetworkIP = '58.71.56';
    ...
    if (typeof ipAddress === 'string' && ipAddress.startsWith(schoolNetworkIP)) {
...
function scan_capture() {
  ...
  axios.post(url, formData, axiosConfig)
    .then(response => {
      ...
      console.log("Dominant Probability:", domProbability + result[0].prediction);
      ...
      document.getElementById("sname").innerHTML = getScientificName(classResult);
      ...
      if (responseReceived === false){
        setTimeout(function() {
          alert("Leaflet took too long to respond. Please check your internet connection.");
        }, 10000);
      }
    })
    .catch(error => {
      ...
      if (axios.isCancel(error)) {
        alert("The request timed out. Please check your internet connection or try again later.");
      } else {
...
function scan() {
    ...
    // Handle the response
    xhr.onload = function() {
      if (xhr.status === 200) // == success: function(result, status)
      { 
    ...
    xhr.onerror = function () {
      ...
      if (xhr.status === 0 && xhr.statusText === "") {
        ...
        alert("Network error: Connection refused. Please check your internet connection or server availability.");
        progressBar.classList.add("d-none");
        btn_scan.style.display = "none";
        btn_scan_again.style.display = "block";
      }
...
var loadImg = function(event){
  ...
  var showBtnScan = function() {
    if ((btn_scan_capture.style.display = "block") && (btn_scan.style.display = "none")){
      btn_scan_capture.style.display = "none";
      btn_scan.style.display = "block";
    }
  }
